%This is the change file for the original Docker's Documentation file.
%This is part of Japanese translation version for Docker's Documantation.

@x 日本語化
<p>Download Docker Desktop</p>
@y
<p>Docker Desktop のダウンロード</p>
@z

@x 日本語化
      href="https://desktop.docker.com/win/main/amd64{{ $build_path }}checksums.txt">checksum</a>) |
@y
      href="https://desktop.docker.com/win/main/amd64{{ $build_path }}checksums.txt">チェックサム</a>) |
@z

@x 日本語化
      href="https://desktop.docker.com/win/main/arm64{{ $build_path }}checksums.txt">checksum</a>) |
@y
      href="https://desktop.docker.com/win/main/arm64{{ $build_path }}checksums.txt">チェックサム</a>) |
@z

@x 日本語化
  <a rel="noopener" marlin-label="download_dmg_click" href="https://desktop.docker.com/mac/main/arm64{{ $build_path }}Docker.dmg">Mac
    with Apple chip</a>
@y
  <a rel="noopener" marlin-label="download_dmg_click" href="https://desktop.docker.com/mac/main/arm64{{ $build_path }}Docker.dmg">Apple チップの Mac</a>
@z

@x 日本語化
      href="https://desktop.docker.com/mac/main/arm64{{ $build_path }}checksums.txt">checksum</a>) |
@y
      href="https://desktop.docker.com/mac/main/arm64{{ $build_path }}checksums.txt">チェックサム</a>) |
@z

@x
  <a rel="noopener" marlin-label="download_dmg_click" href="https://desktop.docker.com/mac/main/amd64{{ $build_path }}Docker.dmg">Mac
    with Intel chip</a>
@y
  <a rel="noopener" marlin-label="download_dmg_click" href="https://desktop.docker.com/mac/main/amd64{{ $build_path }}Docker.dmg">Intel チップの Mac</a>
@z

@x
      href="https://desktop.docker.com/mac/main/amd64{{ $build_path }}checksums.txt">checksum</a>)
@y
      href="https://desktop.docker.com/mac/main/amd64{{ $build_path }}checksums.txt">チェックサム</a>)
@z

@x
      href="https://desktop.docker.com/linux/main/amd64{{ $build_path }}checksums.txt">checksum</a>)
@y
      href="https://desktop.docker.com/linux/main/amd64{{ $build_path }}checksums.txt">チェックサム</a>)
@z
