...
@y
言語別ガイド R 言語向けでは、Docker を使った R 言語アプリケーションのコンテナー化についての説明をはじめます。
...
@y
* R 言語アプリケーションをコンテナー化して実行します。
* コンテナーを利用して R 言語アプリケーションを開発するローカル環境を構築します。
* GitHub アクションを使って、コンテナー化した R 言語アプリケーションの CI/CD パイプラインを設定します。
* コンテナー化した R 言語アプリケーションをローカルな Kubernetes にデプロイして、テストおよびデバッグを行います。
@z

@x
Start by containerizing an existing R application.
@y
では手元にある R 言語アプリケーションのコンテナー化からはじめます。
@z

@x
{{< button text="Containerize a R app" url="containerize.md" >}}
@y
{{< button text="R 言語アプリのコンテナー化" url="containerize.md" >}}
@z
