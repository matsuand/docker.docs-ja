...
@y
For more information, see [Customize an image](__SUBDIR__/dhi/how-to/customize/).
@z
...
@y
`dhictl` is Docker's command-line tool for managing Docker Hardened Images. It lets you browse the DHI catalog, mirror images, and create customizations directly from your terminal. You can integrate `dhictl` into CI/CD pipelines and infrastructure-as-code workflows. You can install `dhictl` as a standalone binary or as a Docker CLI plugin (`docker dhi`); for installation instructions, see [Use the DHI CLI](__SUBDIR__/dhi/how-to/cli/).
@z
...
@y
Use [Docker Debug](__SUBDIR__/dhi/how-to/debug/) if you need to troubleshoot a running distroless container.
@z
