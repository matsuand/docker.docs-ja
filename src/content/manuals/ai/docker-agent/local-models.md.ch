...
@y
エージェントを初めて実行する際に、ローカル環境にモデルがプルされていない場合には、Docker Agent がモデルのプルをプロンプトにより促してきます。
@z
...
@y
DMR の利用にあたってモデルを指定した場合、Docker Agent は自動的にローカル環境にある Docker Model Runner への接続を行い、指示を行うためのインターフェースを確立します。
ローカル環境にモデルがまだ存在せず、それが初めての利用であるとき、Docker Agent がモデルのプルをプロンプトにより促します。
