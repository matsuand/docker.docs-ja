...
1. 端末を開いて Docker Hardened イメージのレジストリにサインインします。
   サインインにあたっては Docker アカウントを利用します。
@z
...
@y
   > [!TIP]
   >
   > Docker アカウントを取得していない場合は、[無償アカウントの生成](https://hub.docker.com/signup) を行ってから進めてください。
@z
...
@y
## 手順 3: 他のイメージとの比較 {#step-3-compare-with-other-images}
@z
...
@y
- [既存アプリの DHI への移行](./migration/migrate-with-ai.md): Gordon を使って、Dockerfile の書き換えを行い、ベースイメージとして Docker Hardened イメージを用いるようにします。
@z
...
@y
- [トライアルをはじめよう](https://hub.docker.com/hardened-images/start-free-trial) によって DHI サブスクリプションのメリット、すなわち FIPS や STIG 準拠へのアクセス、イメージのカスタマイズ、SLA ベースの更新を確認します。
@z
...
@y
- [DHI Select や Enterprise をはじめよう](./how-to/select-enterprise.md):
  DHI サブスクリプションの購入あるいはトライアルを開始した後は、DHI リポジトリのミラー方法について学びます。
  これを行うことにより、リポジトリのミラー、イメージのカスタマイズ、コンプライアンス準拠へのアクセスを学びます。
@z
