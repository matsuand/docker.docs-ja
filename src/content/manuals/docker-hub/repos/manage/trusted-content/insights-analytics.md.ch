...
% __SUBDIR__ ‘Î‰ž / .md ƒŠƒ“ƒN‚Ö‚Ì (no slash) ‘Î‰ž
...
| `INFERRED_USE_CASE` | The inferred deployment environment based on user agent and cloud provider analysis. Values include: <br>â€¢ `Local Dev`: Local development environment (e.g., Docker Desktop, direct `docker` commands) <br>â€¢ `CI/CD`: Continuous integration/deployment pipelines (e.g., containerd, build tools, registry mirroring) <br>â€¢ `Prod`: Production environments (e.g., Kubernetes, container orchestration platforms) <br>â€¢ `Unknown`: Unable to determine the use case from available data |
