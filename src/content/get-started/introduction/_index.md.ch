...
description: |
  ...
  Docker, as well as how to build and push your first image.
summary: |
  Docker とコンテナー技術を理解するためのわかりやすい学習を行います。
  まずは基本的な考え方を理解することとインストール手順を進めます。
  そして Docker コマンド、イメージ生成、コンテナーオーケストレーションを行う実践的な学習を行っていきます。
@z
...
@y
本ガイドシリーズでは Docker を使った実践的な体験を進めていきます。
まずは手元のローカルマシンに Docker Desktop をインストールして設定するところから始めます。
ここからはじめてのコンテナー起動を行って、コンテナー技術の基本やその利点について学びます。
続けて Docker イメージのビルドを通じて、効率の良い再利用可能なイメージを生成する意味を理解します。
そしてそのイメージを Docker Hub に公開する手順を進めることで、より多くのコミュニティ間での成果物の共有や、共同開発およびデプロイに向けた Docker の強力なエコシステム活用を体験していきます。
@z
